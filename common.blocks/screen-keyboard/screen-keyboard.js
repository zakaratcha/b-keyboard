    ...
    provide(bemDom.declBlock(this.name, {
        onSetMod: {
            js: {
                inited: function () {
                    var keyboard = this;
                    var $inner = keyboard.findChildElem('inner').domElem;
                    ...
                    keyboard.domElem[0].onmousedown = function () {
                        return false;
                    };
                    keyboard.domElem[0].onselectstart = function () {
                        return false;
                    };
                    ...
                    var $inputs = $('input, textarea')
                                    ...
                                    .filter(':visible');
                    var onClickOrFocus = keyboard._onClickOrFocus.bind(this);
                    ...
                    keyboard._domEvents().on('click', function (e) {
                        ...
                        if (!isKey && keyboard.targetField) {
                            keyboard.targetField.setCaretPos();
                        }
                    });
        ...
        _keyclick: function (key) {
            ...
            var type = key.getMod('special') || 'normal';
            var keys = keyboard._keys;

            if (key.hasMod('disabled')) {
                return;
            }

            // переключаем режим
            var currMode = keyboard.getMod('mode');
            var keyMode = key.domElem.data('mode');
            var isCurrModeContinuous = keys[currMode].continuous;
            var currRetreat = keys[currMode].retreat || 'normal';
            var newMode = (isCurrModeContinuous && !keyMode) ?
                                                    currMode :
                                                        (keyMode || currRetreat);
        ...
        _keymousedown: function (key) {
            ...
            var keyValue = '';
            var valElem = key.findChildElem('val');
            ...
            keyboard._holdTimeout = setTimeout(function () {
